...
public class NotesManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if(diff.chartFile == null)
        {
            Debug.LogError("chartFileが設定されていません");
            return;
        }
        Load(diff.chartFile); //Load()を呼び出す
    }

    /// <summary>
    /// JSONファイルからノーツデータを読み込み、ノーツを生成する
    /// </summary>
    /// <param name="chartFile">JSONファイル</param>
    private void Load(TextAsset chartFile)
    {
        // 初期化
        laneNum.Clear();
        noteType.Clear();
        ...
        try
        {
            inputJson = JsonUtility.FromJson<Data>(chartFile.text);
        }
        catch (Exception e)
        {
