...
Shader "Skybox/Farland Skies/Low Poly" {		
	...
	Properties{
		...
		//Miss
		_MissIntensity("Miss Intensity", Range(0,1)) = 0
	}
	...
	SubShader{
		...
		Pass{
			...
			v2f vert(appdata v)
			{
				...
				#endif
				#if !CLOUDS_OFF
					OUT.cloudsPosition = RotateAroundYInDegrees(v.vertex, _CloudsRotation + _Time.y * 2.0);
			...
			half4 frag(v2f IN) : SV_Target
			{
				...
				//-----周期(昼夜)-----
				//120秒で1周
				float dayCycle = sin(_Time.y * (2 * UNITY_PI / 120)) * 0.5 + 0.5;
				//夜用(昼 = 0 / 夜 = 1 )
				float nightCycle = 1.0 - dayCycle;
				...
				//夜用の空色(暗め)
				half3 nightTop = _TopColor * 0.2;
				half3 nightMid = _MiddleColor * 0.2;
				half3 nightBottom = _BottomColor * 0.2;

				//昼夜ブレンド
				half3 topCol = lerp(nightTop, _TopColor,dayCycle);
				half3 midCol = lerp(nightMid, _MiddleColor,dayCycle);
				half3 botCol = lerp(nightBottom, _BottomColor,dayCycle);

				// Gradient
				half3 color = lerp(
				lerp(midCol, topCol, pow(t1, _TopExponent)), 
				botCol, 
				pow(t2, _BottomExponent));
				...
				// Stars
					...
					half starsCoef = starsVal * twinklingVal * _StarsTint.a * extinction * nightCycle;
				...
				//Miss演出用カラー(赤紫寄り)
				half3 missColor = half3(0.6,0.1,0.1);

				//Missブレンド
				color = lerp(color, missColor, _MissIntensity * 0.5);
